...
const addToCart = (book) => (dispatch) => {
  dispatch({ type: types.ADD_BOOK_TO_CART_SUCCESS, payload: book });
};

const deleteFromCart = (book) => (dispatch) => {
  dispatch({ type: types.DELETE_BOOK_FROM_CART, payload: book });
};

const confirmChange = (books) => async (dispatch) => {
  dispatch({ type: types.ADD_BOOK_TO_READING_REQUEST, payload: null });
  try {
    const promises = books.map((book) => api.post(`/favorites`, book));
